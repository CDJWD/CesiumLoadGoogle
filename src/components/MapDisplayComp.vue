<template>
    <div id="main">
        ...
        <div class="cesium-viewer-timelineContainer" style="right: 29px; left: 0px;">
            <div class="cesium-timeline-main" id="timecontainer1" @mousedown="timemoveHander_mousemove"
                @mousemove="timemoveHander_mousemove" @mouseup="timemoveHander_mouseup"
                @mouseout="timemoveHander_mouseup">
                <div class="cesium-timeline-bar" style="background-color: blue;position: absolute;" id="timebar1">
                </div>
                <div class="cesium-timeline-bar" style="background-color: red;position: absolute" id="timebar2">
                </div>
...
export default {
    ...
    methods: {
        async init() {
            this.serverurl = config.server.historyImg
            ...
            this.cesiumViewer.camera.setView({
                destination: Cesium.Cartesian3.fromDegrees(104.07, 30.67, 30000.0),
            });
            //显示鼠标坐标
            ...
            //初始化左右控件
            this.initspliteView();
            this.LoadTimesToControl(this.lastaddedtimesArray);
            this.resizeTimeControl();
        ...
        timemoveHander_mousemove(event) {
            if (event.buttons == 0)
                return;
        ...
        timemoveHander_mouseup(event) {
            ...
            console.log('选中时间', currenttimestr, '是否是左边', tagleft)
            if (tagleft)
            ...
            var imageryLayers = this.cesiumViewer.imageryLayers;
            var currentlayer = imageryLayers._layers[tagleft ? 0 : 1];
            imageryLayers.remove(currentlayer, false);
            currentlayer._imageryProvider.baseurl = this.serverurl+  "/getTileImage?x={x}&y={y}&z={z}" + '&time=' + currenttimestr;
            imageryLayers.add(currentlayer, tagleft ? 0 : 1);
            console.log(imageryLayers._layers[0]._imageryProvider.baseurl);
            console.log(imageryLayers._layers[1]._imageryProvider.baseurl);
        },

        showCurretTimeText(tagid, showorhidden, discontent) {
            var label = null;

            // 如果tagid是DOM元素对象，直接使用
            if (typeof tagid === 'object' && tagid !== null) {
                label = tagid;
            } else {
                // 否则按ID查找元素
                label = document.getElementById(tagid);
            }

            // 检查元素是否存在
            if (!label) {
                console.warn('showCurretTimeText: 找不到元素', tagid);
                return;
            }
        ...
        updateTimeLabelPositions() {
            ...
            if (slider) {
                ...
                if (label_left) {
                    label_left.style.left = (slider.offsetLeft / 2) + "px";
                }
                if (label_right) {
                    label_right.style.left = (slider.offsetLeft + (slider.parentElement.offsetWidth - slider.offsetLeft) / 2) + "px";
                }
            }
        },
        ...
        dealSizeChanged() {
            ...
            if (slider == undefined) {
                ...
                this.timemoveHander_mouseup(arg);
            }
        },
        ...
        resizeTimeControl() {
            ...
            var total = slider.parentElement.offsetWidth - 29;

            //定义左边距离右边的位置
            timebar1.style.right = (total - left) + "px";
            timebar1.style.width = (left) + "px";

            timebar2.style.left = (left + sliderwidth) + "px";
            timebar2.style.right = (0) + "px";
            timebar2.style.width = (total - left - sliderwidth - 10) + "px";
        ...
        LoadTimesToControl(timesarray) {
            ...
            if (timesarray == undefined || timesarray.length <= 1) {
                console.log("只获取到时间不足2个",timesarray)
                ...
                var label_left = document.getElementById("timelable_left");
                var label_right = document.getElementById("timelable_right");
                if (label_left) label_left.style.left = "25%";
                if (label_right) label_right.style.left = "75%";
        ...
        ///将时间值显示 到控件上
        SetTimeValues(tagid, timevalues, wantindex) {
            ...
            var times = new Array();
            //选中的时间
            var wantedtime = Date.parse(tagid == "timebar1" ? this.currenttimestrleft : this.currenttimestrright);
            ...
            //总共可用的长度
            var displaylength = controllength - this.blank_length * 2;
            //单个时间的长度
            ...
            for (var i = 0; i < times.length; i++) {
                ...
                ticMain.style.left = (this.blank_length + addlength) + "px";
                ...
                if (addlength - lastaddx >= 100 || i == times.length - 1) {
                    ...
                    ticlabel.style.left = (addlength + this.blank_length / 2) + "px";
                ...
                ticMain.onmouseenter = () => {
                    // 创建或显示工具提示
                    this.showTooltip(ticMain, labelcontent);
                };
                ticMain.onmouseleave = () => {
                    // 隐藏工具提示
                    this.hideTooltip();
                };
                ...
                if (i == wantindex) {
                    ...
                    timemoveHander.style.left = (addlength + this.blank_length - 8) + "px";
                    if (tagid == "timebar1") {
        ...
        initspliteView() {
            ...
            layers.removeAll();
            //右边的图层
            var leftLayer = new GoogleImageryProvider({ baseurl: this.serverurl + "/getTileImage?x={x}&y={y}&z={z}&time=1985-12-30 10:00:00", addone: true });
            var rightlayer = new GoogleImageryProvider({ baseurl: this.serverurl + "/getTileImage?x={x}&y={y}&z={z}&time=2025-12-30 10:00:00", addone: true });
            layers.addImageryProvider(leftLayer);
            var earthAtNight = layers.addImageryProvider(rightlayer);

            earthAtNight.splitDirection = Cesium.SplitDirection.RIGHT;
            // Only show to the left of the slider.
            var slider = document.getElementById('slider');
        ...
        camerachanged: async function (percentage) {
            var extent = this.getCurrentExtent();
            if (extent == null)
                return;
            if(! window.lastgetlevel)
            window.lastgetlevel=13;
            ...
            if(!this.lastaddedtimesArray||this.lastaddedtimesArray.length==0)
            {
                this.currenttimestrleft=timesarray[0];
                this.currenttimestrright=timesarray[timesarray.length-1];
            }
